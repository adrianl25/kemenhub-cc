...
/**
 * Command Center Kemenhub — Dashboard Prototype
 * Next.js (App Router) + TailwindCSS
 * ASCII-only untuk hindari error \uXXXX (smart quotes).
 */
...
type EventItem = {
  id: string; title: string; date: string; location: string; attendedByMinister: boolean;
  source: string; tags?: string[]; summary?: string; link: string;
};
type NewsItem = {
  id: string; title: string; source: string; publishedAt: string; link: string; summary?: string; entities?: string[];
};
type QuoteItem = {
  id: string; text: string; speaker: string; date: string; context?: string; link: string; tags?: string[];
};
...
const sampleEvents: EventItem[] = [
  {
    ...
    summary: "Menhub memimpin rakor dan menekankan zero accident di jalur pelayaran utama.",
    link: "#",
  },
  {
    id: "evt-002",
    ...
    summary: "Peresmian terminal baru untuk meningkatkan konektivitas antarkota di Sulawesi.",
  ...
  },
  {
    ...
    summary: "Staf ahli Kemenhub mewakili Menhub membahas roadmap transportasi rendah emisi.",
    link: "#",
  },
];
...
const sampleNews: NewsItem[] = [
  {
    id: "news-101",
    title: "Menhub Dorong Integrasi Moda untuk Kurangi Kemacetan di Metropolitan",
    source: "Antara",
    publishedAt: daysAgo(0).toISOString(),
    link: "https://www.antaranews.com/",
    summary: "Dalam keterangan pers, Menhub menyoroti pentingnya integrasi antarmoda dan tiket terusan.",
  ...
  },
  {
    ...
    link: "#",
    summary: "Regulasi baru menekankan audit berkala dan peningkatan pelatihan bagi maskapai.",
  ...
  },
  {
    ...
    link: "#",
    summary: "Pilot project bus listrik memasuki fase pengembangan infrastruktur pengisian daya.",
...
const sampleQuotes: QuoteItem[] = [
  {
    id: "q-01",
    text: "Keselamatan adalah prioritas utama - tidak boleh ada kompromi di darat, laut, maupun udara.",
    speaker: "Menteri Perhubungan",
    date: daysAgo(0).toISOString(),
    context: "Pernyataan di rapat koordinasi keselamatan pelayaran, Jakarta.",
    link: "#",
    tags: ["Keselamatan", "Kebijakan"],
  },
  {
    id: "q-02",
    text: "Integrasi antarmoda bukan pilihan, melainkan kebutuhan kota-kota besar Indonesia.",
    speaker: "Menteri Perhubungan",
    date: daysAgo(1).toISOString(),
    context: "Keterangan pers mengenai integrasi moda di wilayah metropolitan.",
    link: "https://www.dephub.go.id/",
    tags: ["Integrasi Moda", "Perkotaan"],
  },
  {
    id: "q-03",
    text: "Transisi ke transportasi rendah emisi harus disertai skema pendanaan yang berkelanjutan.",
    speaker: "Menteri Perhubungan",
...
function filterEvents(data: EventItem[], { minDate, onlyMinister, tag, query }: { minDate: Date; onlyMinister: boolean; tag: string; query: string; }) {
  return data
...
function filterNews(data: NewsItem[], { minDate, tag, query }: { minDate: Date; tag: string; query: string; }) {
  return data
...
function filterQuotes(data: QuoteItem[], { minDate, tag, query }: { minDate: Date; tag: string; query: string; }) {
  return data
...
export default function Page() {
  ...
  const pushToast = (message: string, type: "success"|"info"|"warning"|"error" = "info", duration = 3000) => {
  ...
  const tagsUniverse = useMemo(() => {
    ...
    [...sampleEvents, ...sampleNews, ...sampleQuotes].forEach((it) => {
      (it as any).tags?.forEach((t: string) => set.add(t));
      (it as any).entities?.forEach((t: string) => set.add(t));
    });
    return [...set];
  }, []);

  const filteredEvents = useMemo(() => filterEvents(sampleEvents, { minDate, onlyMinister, tag, query }), [minDate, onlyMinister, tag, query]);
  const filteredNews   = useMemo(() => filterNews(sampleNews,       { minDate, tag, query }), [minDate, tag, query]);
  const filteredQuotes = useMemo(() => filterQuotes(sampleQuotes,   { minDate, tag, query }), [minDate, tag, query]);

  const handleGenerateCaption = () => {
    const top: any = filteredNews[0] || filteredEvents[0];
    if (!top) { pushToast("Tidak ada item untuk dijadikan caption", "warning", 2500); return; }
    const base = `Menhub: ${top.title || "Pembaruan kegiatan hari ini"} - ${top.summary || top.source || "Info terbaru"}. #Kemenhub #Transportasi`;
    setCaption(base);
    pushToast("Caption dibuat", "success", 1800);
  };

  const handleCopy = async () => {
    if (!caption) return;
    const ok = await copyToClipboard(caption);
    if (ok) pushToast("Caption disalin ke clipboard", "success", 2000);
    else pushToast("Gagal menyalin clipboard. Coba pilih teks lalu Ctrl/Cmd+C.", "error", 3000);
  };

  const handleLinkClick = (ev: React.MouseEvent, link: string) => {
    if (link === "#") {
      ev.preventDefault();
      pushToast("Sumber belum tersedia", "warning", 2000);
    }
  };
  ...
  return (
    <div className={classNames(
      "min-h-screen",
      darkMode ? "bg-gradient-to-br from-slate-800 via-slate-900 to-slate-700 text-slate-100"
      ...
      <button
        ...
        className={classNames(
          ...
          darkMode ? "bg-slate-800 border-slate-600 text-slate-100 hover:bg-slate-700"
      ...
      {/* Top Bar */}
      <header className={classNames("sticky top-0 z-20 border-b backdrop-blur",
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2 sm:py-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            <div className={classNames("w-9 h-9 rounded-xl grid place-items-center font-bold",
              darkMode ? "bg-indigo-500 text-white" : "bg-indigo-700 text-white")}>CC</div>
            <div>
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button onClick={() => setAsideOpen(true)} className={classNames("lg:hidden px-3 py-2 rounded-full text-sm border",
                darkMode ? "bg-slate-800 border-slate-600 text-slate-100" : "bg-white border-slate-300 hover:bg-slate-50")}>Panel Info</button>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 py-5 grid grid-cols-12 gap-4 sm:gap-6">
        ...
        <section className="col-span-12 lg:col-span-8 space-y-4 sm:space-y-6">
          ...
          <Card dark={darkMode}>
            <div className="flex flex-col md:flex-row md:flex-wrap items-start md:items-center gap-3">
              ...
              <div className="flex items-center gap-2 w-full md:w-auto">
                ...
                <div className="flex items-center gap-2">
                  <span className={classNames("text-xs px-2 py-0.5 rounded-full border",
                    darkMode ? "bg-slate-700 border-slate-600" : "bg-slate-100 border-slate-200") }>
                    {tag === "All" ? `All (${tagsUniverse.length - 1})` : tag}
                  </span>
                  <button onClick={() => setTagsOpen(true)} className={classNames("px-2 py-1 rounded-md text-xs border",
            ...
            <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
              <Card dark={darkMode}>
                <SectionHeader
                  ...
                  right={<button className={classNames("text-sm underline-offset-2", darkMode ? "text-indigo-300 hover:underline" : "text-indigo-700 hover:underline")} onClick={()=>setTab("events")}>Lihat semua</button>}
                />
                <div className="space-y-4">
                  ...
                  {filteredEvents.length === 0 && <Empty msg="Tidak ada event dalam rentang waktu ini." />}
                </div>
              </Card>
              ...
              <Card dark={darkMode}>
                <SectionHeader
                  icon={<span className={classNames("w-2.5 h-2.5 rounded-full inline-block", darkMode ? "bg-indigo-400" : "bg-indigo-700")} />}
                  title="News Ringkas"
                  right={<button className={classNames("text-sm underline-offset-2", darkMode ? "text-indigo-300 hover:underline" : "text-indigo-700 hover:underline")} onClick={()=>setTab("news")}>Lihat semua</button>}
                />
                <div className="space-y-4">
              ...
              <Card dark={darkMode}>
                <SectionHeader
                  ...
                  right={<button className={classNames("text-sm underline-offset-2", darkMode ? "text-indigo-300 hover:underline" : "text-indigo-700 hover:underline")} onClick={()=>setTab("quotes")}>Lihat semua</button>}
                />
                <div className="space-y-4">
              ...
              <Card dark={darkMode}>
                <SectionHeader icon={<span className={classNames("w-2.5 h-2.5 rounded-full inline-block", darkMode ? "bg-amber-400" : "bg-amber-500")} />} title="Generator Caption (Manual Stub)" />
                <div className="space-y-2">
                  <div className="text-sm opacity-80">Ambil item teratas dari News atau Events setelah filter, lalu buat caption cepat.</div>
                  <div className="flex flex-wrap gap-2">
                    <button onClick={handleGenerateCaption} className={classNames("px-3 py-2 rounded-xl", darkMode ? "bg-indigo-500 text-white hover:bg-indigo-600" : "bg-indigo-700 text-white hover:bg-indigo-800")}>Generate</button>
                    <button onClick={handleCopy} disabled={!caption} className={classNames("px-3 py-2 rounded-xl border", darkMode ? "border-slate-600 hover:bg-slate-800" : "border-slate-300 hover:bg-slate-50", !caption && "opacity-50 cursor-not-allowed")}>Copy</button>
                    {caption && <StatBadge>Siap diposting</StatBadge>}
                  </div>
                  <textarea className={classNames("w-full h-28 p-3 rounded-xl border focus:outline-none",
                    darkMode ? "bg-slate-800 border-slate-600 text-slate-100 focus:ring-2 focus:ring-indigo-500" : "border-slate-300 focus:ring-2 focus:ring-indigo-600")}
          ...
          {tab === "events" && (
            <Card dark={darkMode}>
              <SectionHeader icon={<span className={classNames("w-2.5 h-2.5 rounded-full inline-block", darkMode ? "bg-indigo-400" : "bg-indigo-700")} />} title="Daftar Events" right={<div className="text-sm flex items-center gap-2"><StatBadge>{filteredEvents.length} item</StatBadge></div>} />
              <div className="divide-y" style={{ borderColor: darkMode ? "#334155" : "#e2e8f0" }}>
                ...
                {filteredEvents.length === 0 && <Empty msg="Tidak ada event ditemukan." />}
              </div>
            </Card>
          )}
          ...
          {tab === "news" && (
            <Card dark={darkMode}>
              <SectionHeader icon={<span className={classNames("w-2.5 h-2.5 rounded-full inline-block", darkMode ? "bg-indigo-400" : "bg-indigo-700")} />} title="Berita Terbaru" right={<div className="text-sm flex items-center gap-2"><StatBadge>{filteredNews.length} item</StatBadge></div>} />
              <div className="divide-y" style={{ borderColor: darkMode ? "#334155" : "#e2e8f0" }}>
                {filteredNews.map((n) => (
                ...
                {filteredNews.length === 0 && <Empty msg="Belum ada berita ditemukan." />}
              </div>
            </Card>
          )}
          ...
          {tab === "quotes" && (
            <Card dark={darkMode}>
              <SectionHeader icon={<span className={classNames("w-2.5 h-2.5 rounded-full inline-block", darkMode ? "bg-indigo-400" : "bg-indigo-700")} />} title="Kutipan Penting" right={<div className="text-sm flex items-center gap-2"><StatBadge>{filteredQuotes.length} item</StatBadge></div>} />
          ...
          <div className="hidden lg:block">
            <Card dark={darkMode}>
              ...
              <ul className="text-sm space-y-2">
                <li>Agenda Resmi: <span className="text-emerald-500 font-medium">OK</span> - 30 menit lalu</li>
                <li>Media Sosial: <span className="text-emerald-500 font-medium">OK</span> - 28 menit lalu</li>
                <li>Portal Berita: <span className="text-amber-500 font-medium">Terjadwal</span> - 10 menit lagi</li>
              </ul>
            </Card>
      ...
      {tagsOpen && (
        <div className="fixed inset-0 z-40">
          ...
          <div className={classNames("absolute right-0 top-0 h-full w-96 max-w-[90%] shadow-2xl p-4 overflow-y-auto", darkMode ? "bg-slate-800" : "bg-white") }>
            ...
            <div className="mt-4">
              <button onClick={() => { setTag("All"); setTagsOpen(false); }} className={classNames("px-3 py-2 rounded-xl border text-sm", darkMode ? "bg-slate-800 border-slate-600" : "bg-white border-slate-300 hover:bg-slate-50")}>Reset ke All</button>
            </div>
          </div>
        </div>
      )}
      ...
      {toasts.length > 0 && (
        <div className="fixed bottom-4 right-4 z-50 space-y-2">
          {toasts.map((t) => (
            <div key={t.id}
              className={
                "text-sm px-4 py-3 rounded-lg shadow-lg text-white flex items-center gap-3 " +
                (t.type === "success" ? "bg-emerald-600" : t.type === "warning" ? "bg-amber-600" : t.type === "error" ? "bg-rose-600" : "bg-slate-900")
              }>
              <span>{t.message}</span>
              <button className="text-white/80 hover:text-white text-xs border border-white/40 rounded px-2 py-0.5" onClick={() => removeToast(t.id)}>Tutup</button>
            </div>
          ))}
        </div>
      )}
